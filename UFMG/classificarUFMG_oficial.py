...
def categoria_ufmg(g):
    try:
        comeco = g[g.find('-')+1:]
        c = comeco[comeco.find('-')+1:]
        f = c[:c.find('-')]
        return f
    except:
        return f
...
while i<= len(lista_patente_link):
    try:
        ...
        #oferta tecnologica
        elif str(lista_patente_link[i][1]).find('Oferta Tecnológica') != -1 or str(lista_patente_link[i][1]).find('oferta Tecnológica') != -1 or str(lista_patente_link[i][1]).find('EDITAL') :
        ...
        #distrato
        elif str(lista_patente_link[i][1]).find('Distrato ') != -1 or str(lista_patente_link[i][1]).find('Rescisão') != -1:
        ...
        #acordo de parceria
        elif str(lista_patente_link[i][1]).find('Acordo de Parceria') != -1 or str(lista_patente_link[i][1]).find('Intenção de Parceria') != -1:
        ...
        #licenciamento
            ...
            if str(lista_patente_link[i][1]).find('Cooperação') != -1 or str(lista_patente_link[i][1]).find('cooperação') != -1:
                lista_patente_link[i].append('Cooperação tecnica')
            else:
...
for element in lista_patente_link:
    if element not in resultantList:
        resultantList.append(element)

print(resultantList)
''', retornar_patent(resultantList[i][1]), encontrar_vigencia(resultantList[i][1]), encontrar_dmy(resultantList[i][1]), cnpj(resultantList[i][1])'''
for i in range(len(resultantList)):
    resultantList[i].append(categoria_ufmg(resultantList[i][1]))
    resultantList[i].append(retornar_patent(resultantList[i][1]))
    resultantList[i].append(encontrar_vigencia(resultantList[i][1]))
    resultantList[i].append(encontrar_dmy(resultantList[i][1]))
    resultantList[i].append(cnpj(resultantList[i][1]))
df = pd.DataFrame(resultantList, columns=['Link', 'Conteudo', 'classificacao','codigo', 'vigência', 'data_assinatura', 'cnpj'])
